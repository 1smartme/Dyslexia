...
export default function ParentLogin() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.message || "Parent login failed");
        return;
      }
  ...
  return (
    <div className="min-h-screen gradient-bg flex items-center justify-center p-4">
      <div className="card w-full max-w-md p-8">
        ...
        <p className="text-sm text-gray-600 mb-6">Track your child learning progress securely.</p>
        ...
        <div className="mt-4 text-sm">
          No parent account? <Link className="text-primary-600" to="/parent/signup">Create one</Link>
        </div>
      </div>
    </div>
  );
